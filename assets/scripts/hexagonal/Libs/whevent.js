...
window.director = {
  ...
  unbind: function (signal, func, self) {
    ...
    for (var i = 0; i < this._callStacks[signal].length; i++) {
      if (
        this._callStacks[signal][i].func === func &&
        (!self || this._callStacks[signal][i].self === self)
      ) {
...
// Aliases
director.on = director.bind;
director.onOnce = director.bindOnce;
director.onPriority = director.bindPriority;
director.onOncePriority = director.bindOncePriority;
director.off = director.unbind;
director.emit = director.call;
